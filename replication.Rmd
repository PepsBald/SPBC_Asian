...
table(bc2$Sequence_number, useNA="always")
   ...
   ##################################################################   
      ...
      many.bc1 <- rbind(many.bc_first, many.bc_multi)
              ...
              ## spbc: a subset of a1, which includes pts with multiple PBCs, 
              ## with survival > 6 months and laterality different in between PBC diagnosises;
                  spbc <- subset(a1, t.diff > 6 & laterality.differ == 1); dim(a1); dim(spbc); length(unique(spbc$Patient_ID))
                  # [1] 106279    252
                  # [1] 40197   252
                  # [1] 39426
              ...
              ## spbc2: subset of spbc2 taking out all duplicates, only most recent spbc is kept
...
# -250.00  -18.00   21.00   25.74   80.00  250.00     225 
      ...
      ## spc: all cases with other SPC combined with bc info conditioned on bc first occurrence
       spc <- subset(oc2, t.diff > 6); dim(oc2); dim(spc); length(unique(spc$Patient_ID))
        # [1] 150768    249
        # [1] 87526   249
        # [1] 79020
      ...
      ## ss: all secondary cancers, spbc2 and spc1
       ss <- rbind(spbc2, spc1); dim(spbc2); dim(spc1); dim(ss)
          ...
          # [1] 87526     6
          # [1] 126952      6  
       length(unique(ss$Patient_ID)) # [1] 113754
       
       ss <- ss[order(ss$Patient_ID, ss$Survival_months1),]; View(ss)
       
      ## ss1: all secondary cancers, spbc2 and spc1, removing duplicates with same id and icd.sx
       ss1 <- ss[!duplicated(ss[c("Patient_ID", "icd.sx")]),]; dim(ss); dim(ss1) 
        #        [1] 126952      6
        # [1] 124154      6

       head(ss1,10)
            #     Patient_ID age.dx1 Survival_months1 yr.dx1 icd.sx
            # 3          374      83               87   2013    C50
            # 6         1037      64               10   2020    C44
            # 7         1907      56              148   2008    C56
            # 810       2605      63               47   2017    C54 <-- 
            # 4         2605      61               70   2015    C50 <-- 
            # 11        3909      87                8   2016    C49
            # 122       5019      90                0   2017    C80
            # 15        5597      90               57   2015    C44
            # 17        6377      64               13   2019    C20
            # 8         7723      72                7   2020    C50
